...
## Correção pós-fase — coluna `title` (2026-07-28)

Feita depois de a fase fechar, ao conferir a entrega contra o **enunciado do desafio** e não apenas contra o `project-plan.md`.

- **A lacuna.** O Data Model original declarava `title` e `description` como *deliberately absent*, argumentando que metadado editável de vídeo é da Fase 04. O argumento saiu da lista de capabilities do `project-plan.md`. Mas o enunciado nomeia `título` entre os campos mínimos de persistência — *"com pelo menos identificação, dono (canal), título, status, chaves de storage do arquivo e do thumbnail, duração e metadados, e o identificador da URL única"*. Ou seja: o campo **tinha** origem rastreável, e a ausência era lacuna, não decisão de escopo. Foi o único ponto da entrega onde um avaliador conseguiria apontar uma palavra do enunciado ausente do código.
- **O que entrou.** Coluna `varchar(255) NOT NULL`, migration `AddVideoTitle1785209037081`, campo **opcional** no `InitiateUploadDto` e presente no `VideoResponseDto`. Sem endpoint de edição — isso continua sendo Fase 04.
- **Por que opcional.** A capability é o pré-cadastro *automático* do rascunho. Exigir título na iniciação quebraria um cliente que só tem o arquivo. Ausente ou em branco, deriva de `original_filename` sem a extensão, com fallback `video`.
- **A migration não é a que o CLI gerou.** O CLI emitiu `ADD "title" ... NOT NULL` puro, que quebra em qualquer tabela com linhas. Virou add nullable → backfill → `SET NOT NULL`, que é a forma de data migration que as regras do projeto permitem escrever à mão. O backfill usa a mesma regra do serviço, então linha criada antes e depois da migration fica indistinguível.
- **O gate pegou dois defeitos meus.** Primeiro, cinco specs semeavam `Video` direto pelo repositório e passaram a violar o `NOT NULL`. Segundo, e mais sério: `migrations.integration-spec.ts` mantém **sua própria lista explícita de migrations**, que eu não atualizei — mesma classe do `global-setup.ts`. Como esse spec derruba a tabela `migrations` e reaplica só o que está na sua lista, ele deixava o banco de teste com 3 migrations registradas e o schema real na 4ª. O `test:e2e` seguinte tentava aplicar `AddVideoTitle` de novo e morria com *"column already exists"*.
- **A deriva tinha um segundo motor:** `createTestDataSource` usa `synchronize: true` por padrão, então qualquer spec de integração materializa a coluna a partir da entidade sem registro de migration nenhum. Por isso a coluna existia com o bookkeeping em 3. A migration ganhou `IF NOT EXISTS`/`IF EXISTS` — que é regra explícita do projeto para DDL — e o spec ganhou comentário dizendo que toda migration em disco precisa estar na lista.
- **Verificação:** banco de teste reparado à mão uma vez (a coluna órfã removida), depois `9/9` gates verdes e **uma segunda rodada consecutiva** do gate de backend, já que o defeito inteiro era estado vazando entre execuções. `openapi.json` regerado: `title` aparece no `InitiateUploadDto` fora do `required` e no `VideoResponseDto`.
