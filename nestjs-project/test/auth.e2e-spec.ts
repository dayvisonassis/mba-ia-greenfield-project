...
/**
 * Supertest types `Response.body` as `any`. These describe the shapes this
 * suite actually asserts on, so a renamed field fails `tsc --noEmit` instead of
 * silently comparing `undefined`.
 *
 * Field names mirror the API contract (snake_case tokens) — see openapi.json.
 */
...
function body(res: { body: unknown }): AuthResponseBody {
...
describe('Auth (e2e)', () => {
  ...
  async function captureConfirmationToken(
    ...
    jest
      ...
      .mockImplementationOnce((_e: string, _n: string, t: string) => {
        capturedToken = t;
        // Returns a Promise explicitly: the real method is Promise<void>, and
        // dropping `async` without this would change the mock's return type.
  ...
  async function registerConfirmAndLogin(
    ...
    const res = await request(app.getHttpServer())
      .post('/auth/login')
      .send({ email, password });
    // A successful login always carries both tokens — narrowed here so callers
    // get `string` instead of `string | undefined`.
