...
// Only the EntityManager methods ChannelsService actually calls. Typed so a
// renamed call site fails `tsc --noEmit` instead of silently mocking nothing.
...
function makeManager(overrides: Record<string, jest.Mock> = {}): MockManager {
  return {
    findOne: jest.fn(),
    create: jest.fn(),
    save: jest.fn(),
...
function makeUniqueError(): QueryFailedError {
  // `code` is the pg SQLSTATE as a string ('23505'), matching the production
  // check in channels.service.ts — never a number.
